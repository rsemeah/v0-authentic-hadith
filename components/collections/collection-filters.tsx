...
export function CollectionFilters({ onFiltersChange, scholars }: CollectionFiltersProps) {
  ...
  const FilterContent = () => (
    <div className="space-y-6">
      {/* Grade Filter */}
      <div>
        <h4 className="text-sm font-semibold text-[#1a1f36] mb-3">Grade</h4>
        <div className="space-y-2">
          {grades.map((grade) => (
            <label key={grade.id} className="flex items-center gap-3 cursor-pointer group">
              <div
                className={cn(
                  ...
                  selectedGrades.includes(grade.id)
                    ...
                    : "border-[#d4cfc7] group-hover:border-[#C5A059]",
                )}
              >
                {selectedGrades.includes(grade.id) && (
              ...
              <span className="text-sm text-[#1a1f36]">{grade.label}</span>
      ...
      {/* Scholar Filter */}
      <div>
        <h4 className="text-sm font-semibold text-[#1a1f36] mb-3">Scholar</h4>
        <div className="space-y-2 max-h-[200px] overflow-y-auto">
          {scholars.map((scholar) => (
            <label key={scholar} className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-[#1a1f36] truncate">{scholar}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      {/* Sort */}
      <div>
        <h4 className="text-sm font-semibold text-[#1a1f36] mb-3">Sort By</h4>
        <div className="relative">
          <select
            value={sortBy}
            onChange={(e) => handleSortChange(e.target.value)}
            className="w-full px-3 py-2 rounded-lg border border-[#d4cfc7] bg-white text-sm appearance-none cursor-pointer focus:border-[#C5A059] focus:outline-none"
          >
      ...
      {activeFilterCount > 0 && (
        <button
          ...
          className="w-full py-2 rounded-lg border border-[#d4cfc7] text-sm text-muted-foreground hover:border-[#C5A059] hover:text-[#C5A059] transition-colors"
        >
          Clear All Filters
        </button>
      )}
    ...
    <>
      ...
      <aside className="hidden md:block w-[240px] flex-shrink-0 sticky top-[80px] self-start">
        <div className="p-5 rounded-xl premium-card">
          <h3 className="text-base font-bold text-[#1a1f36] mb-4 flex items-center gap-2">
      ...
      >
        ...
        Filters
        {activeFilterCount > 0 && (
          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-white/20">{activeFilterCount}</span>
        )}
      </button>
      ...
      {showMobileFilters && (
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-[#F8F6F2] rounded-t-3xl p-6 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-[#1a1f36]">Filters</h3>
              <button
