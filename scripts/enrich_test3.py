...
def sb(method, path, data=None):
    ...
    return http_req(method, SB_URL + "/rest/v1/" + path, data, h)

print("Step 1: Fetching 3 unenriched hadiths...")
enriched = sb("GET", "hadith_enrichment?select=hadith_id&limit=50000")
done_ids = set(e["hadith_id"] for e in enriched)
print(f"  Already enriched: {len(done_ids)}")

hadiths = sb("GET", "hadiths?select=id,english_translation,narrator,grade&order=hadith_number.asc&limit=100")
todo = [h for h in hadiths if h["id"] not in done_ids][:3]
print(f"  Got {len(todo)} to process")
...
JSON fields:
- summary_line: 5-12 word English summary
- key_teaching_en: 2-4 sentences. First accessible, then scholarly with scholar refs
- key_teaching_ar: Arabic MSA translation of key_teaching_en
- summary_ar: 3-10 word Arabic summary
- category_slug: ONE of [{cat_list}]
- tag_slugs: 1-4 of [{tag_list}]
- confidence: float 0-1

Return ONLY valid JSON, no markdown."""

    resp = http_req("POST", "https://api.deepinfra.com/v1/openai/chat/completions", {
        "model": "meta-llama/Llama-3.3-70B-Instruct",
        "messages": [
            {"role": "system", "content": "You are a hadith scholar. Return only valid JSON."},
    ...
    obj = json.loads(raw)
    print(f"  Summary: {obj.get('summary_line','?')}")
    print(f"  Category: {obj.get('category_slug','?')}")
    print(f"  Teaching: {(obj.get('key_teaching_en','?'))[:80]}...")
    ...
    print(f"  Inserted enrichment: {ins[0]['id'][:8] if ins else 'FAILED'}")
