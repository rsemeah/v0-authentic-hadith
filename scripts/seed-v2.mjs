...
/**
 * Simple Hadith Seeding Script v2
...
async function seedCollection(coll) {
  ...
  for (let section = 1; section <= 200; section++) {
    const engData = await fetchJSON(`${CDN}/${coll.eng}/${section}.min.json`);
    if (!engData) {
      consecutiveMisses++;
    ...
    if (!engData.hadiths?.length) continue;
    
    const araData = await fetchJSON(`${CDN}/${coll.ara}/${section}.min.json`);
    ...
    // Filter out existing
    const newHadiths = engData.hadiths.filter(h => !existingNums.has(h.hadithnumber));
    ...
    for (let i = 0; i < newHadiths.length; i += 50) {
      ...
      if (error) {
        log(`⚠️  Batch error sec ${section}: ${error.message}`);
        errors++;
        continue;
      }
      
      if (inserted?.length) {
        // Link to collection (skip book_id and chapter_id)
    ...
    process.stdout.write(`[${section}:+${newHadiths.length}]`);
  }
  ...
  log(`\n✅ DONE: +${totalInserted} inserted | Total: ${count}`);
...
async function main() {
  ...
  console.log("\n🌙 Authentic Hadith - Simple Seeding Script v2");
